...
class Trajectory(object):
    ...
    def __init__(self, sys, **kwargs):
        ...
        # chains=[class ic_1, class ic_2], eqind=[3] means x4,
        # ic_1: x1->x2->u1; ic_2: x3->x4
...
#        if self.splines == {}: # the first round
#            n_round = True
#        else: # not the first round
#            n_round = False
        # self._old_par = copy.deepcopy().
        
        bv = self.sys.boundary_values # self.sys==sys, sys comes from class collocation, from ControlSystem, from DynamicSystem, has the attr boundary.value.
        # bv = {'x2': (0.0, 0.0), 'x3': (1.2566370614359172, 0.6283185307179586), 'x1': (0.0, 0.6283185307179586), 'u1': (0.0, 0.0), 'x4': (0.0, 0.0)}
        ...
        if self._parameters['use_chains']: # self._chains=[class ic_1, class ic_2]
            ...
            for chain in self._chains:
                ...
                if chain.lower.startswith('x'):
                    ...
                    # splines={'x3': Spline object, 'x1': Spline object}
                ...
                for i, elem in enumerate(chain.elements): # chain.elements= ('x1', 'x2', 'u1') or ('x3','x4')
                    if elem in self.sys.states:
                        ...
                        # for (x3,x4): splines['x3']._b_v= {0: (1.2566370614359172, 0.6283185307179586), 1: (0.0, 0.0)}
                        # 0 is for x3, 1 is for x4, there is only splines['x3'],without splines['x4'], because upper here is only 'x3'
                ...
                for i, elem in enumerate(chain.elements):
                    if elem in self.sys.inputs:
                        ...
                        if (i == 2): # because of elements=('x1','x2','u1'), (i=2,elem=u1)
                            ...
                            # u_fnc={'u1': method Spline.ddf}
                    elif elem in self.sys.states: # for (0,'x1')
                        if (i == 0):
                            splines[upper]._boundary_values[0] = bv[elem]
                            if splines[upper].type == 'u': # lower contains u
                        ...
                        # x_fnc={'x1': method Spline.f, x2': Spline.df, 'x3': Spline.f, 'x4': Spline.df}
                        
        # now handle the variables which are not part of any chain
        for i, xx in enumerate(self.sys.states): # enumerate return key and value # ('x1',...,'xn')
        ...
        # dx_fnc={'x1': method Spline.df, 'x2': Spline.ddf, 'x3': Spline.df, 'x4': Spline.ddf}
        indep_coeffs = dict()
        for ss in splines.keys(): # because key of dict(splines) is only 'upper' (splines[upper])
        # splines{'x1': class Spline, 'x3': class Spline}
            indep_coeffs[ss] = splines[ss]._indep_coeffs
        # array([cx1_0_0, cx1_1_0, cx1_2_0, ..., cx1_14_0, cx1_15_0, cx1_16_0]), it should have 20 coeff. but only 17 are independent.
        # array([cx3_0_0, cx3_1_0, cx3_2_0, ..., cx3_8_0])
#        if n_round == True:
        indep_coeffs['z_par'] = np.array([sp.symbols('k')])
#        elif n_round == False:
#            indep_coeffs['z_par'] = 0

        self.indep_coeffs = indep_coeffs
        self.splines = splines
        self.x_fnc = x_fnc
        # x_fnc
# ={'x2': <bound method Spline.f of <pytrajectory.splines.Spline object >>, 'x3': <bound method Spline.f of <pytrajectory.splines.Spline object>>, 'x1': <bound method Spline.f of <pytrajectory.splines.Spline object>>, 'x4': <bound method Spline.f of <pytrajectory.splines.Spline object>>}
        self.u_fnc = u_fnc
        self.dx_fnc = dx_fnc
    ...
    def set_coeffs(self, sol):
        ...
        for k, v in sorted(self.indep_coeffs.items(), key=lambda (k, v): k):
            # {'x3': array([cx3_0_0, cx3_1_0, cx3_2_0, cx3_3_0, cx3_4_0, cx3_5_0, cx3_6_0, cx3_7_0, cx3_8_0], dtype=object), 
               # 'x1': array([cx1_0_0, cx1_1_0, cx1_2_0, ..., cx1_14_0, cx1_15_0, cx1_16_0], dtype=object)}
            i = len(v)
        ...
        for k in self.splines.keys(): # ['x1','x3']
            ...
            # self._indep_coeffs = free_coeffs (self.splines[k]._indep_coeffs=free_coeffs) makes symbols changing into numbers.
            # {'x1': <Spline object>, 'x3': <Spline object>}, Spline._P[k] saves the polynomial.
...
#        self.indep_coeffs = indep_coeffs
        ...
        # {'x1': array([ 25.94485709,  16.38313857, -35.65010072, ...,   2.28427004,
        # 2.82974712,   1.88490863])
        #'x3': array([-34.33884269,  45.13959025,   1.3272378 ,  -4.15546318,
        # 5.3863866 ,  -5.39286006,  -8.86559812,  -6.11620983,  -2.95630206])}
