# Beschränkung von k in Inversen Pendel System
...
S = ControlSystem(f, a, b, xa, xb, ua, ub, su=2, sx=2, kx=2, use_chains=False, maxIt=10, constraints=con, par=par, dt_sim=0.001) #constraints=con,
...
if plot:
    ...
    for i in ax:
        ...
        plt.grid()
        if i == 0:
            plt.ylabel(r'$x_{}(m)$'.format(i + 1), fontsize=46)
        if i == 1:
            plt.ylabel(r'$x_{}(m/s)$'.format(i + 1), fontsize=46)
        if i == 2:
            plt.ylabel(r'$x_{}(rad)$'.format(i + 1), fontsize=46)
        if i == 3:
            plt.ylabel(r'$x_{}(rad/s)$'.format(i + 1), fontsize=46)
    ...
    plt.xlabel(r'$t(s)$', fontsize=46)
    plt.ylabel(r'$u(m/s^2)$', fontsize=46)
    plt.grid()
    plt.sca(ax2)
    plt.plot(range(len(S.k_list)), S.k_list, '.',markersize = 10, linewidth=3.0)
    plt.xlabel(r'$Iteration$', fontsize=46)
