...
Mit Eingangstrajektorien aus Pytrajectory das Steuergesetz mit linearen Interpolation entwerfen
'Path' braucht zu verändern
"""
import pickle
import numpy as np

path = 'G:\DA\Codes\Data\without_Refsol_Brockett\Brockett_e2\delta_x_0.01'
...
if get_u_list:
    ...
    for i in range(len(x_1)):
        plt.plot(t_data, u1_new[i], linewidth=3.0, label=r'$x_{}|0={}$'.format(1,x_1[i]))
        plt.grid(True)
    plt.xlabel(r'$t(s)$', fontsize=58)
    plt.ylabel(r'$u_{1}$', fontsize=58)

    plt.subplot(1, 2, 2)
    for i in range(len(x_1)):
        plt.plot(t_data, u2_new[i], linewidth=3.0, label=r'$x_{}|0={}$'.format(1, x_1[i]))
        plt.grid(True)
    plt.xlabel(r'$t$', fontsize=58)
    plt.ylabel(r'$u_{2}$', fontsize=58)

    plt.figure(2)
    plt.subplot(2, 2, 1)
    for i in range(len(x_1)):
        plt.plot(t_data, res_1[i][:-1, 0], linewidth=3.0, label=r'$x_{}|0={}$'.format(1, x_1[i]))
        plt.grid(True)
    plt.xlabel(r'$t$', fontsize=46)
    plt.ylabel(r'$x_{1}$', fontsize=46)

    plt.subplot(2, 2, 2)
    for i in range(len(x_1)):
        plt.plot(t_data, res_1[i][:-1, 1], linewidth=3.0, label=r'$x_{}|0={}$'.format(1, x_1[i]))
        plt.grid(True)
    plt.xlabel(r'$t$', fontsize=46)
    ...
    plt.subplot(2, 2, 3)
    for i in range(len(x_1)):
        plt.plot(t_data, res_1[i][:-1, 2], linewidth=3.0, label=r'$x_{}|0={}$'.format(1, x_1[i]))
        plt.grid(True)
    plt.xlabel(r'$t$', fontsize=46)
