# -*- coding: utf-8 -*-
# Untersuchung der Regelmäßigkeit der Eingangstrajektorien mit dem Anfangswert von x1 von -0.06 bis -0.03 mit der Schrittweite h=0.005 zu verändern und x2,0=x3,0=0.1 für Brocketts Beispiel
...
xa_start = [-0.06,0.1,0.1]          # if x1:-0.01 -> 0   (xa_start_new[0]<=0.001)
delta_xx_1 = np.array([0.005*1, 0, 0])
...
if 1:
    if 1:
        if 1:
            for d1 in range(int((-0.03 - (xa_start[0])) / delta_xx_1[0]) + 1):
    ...
    # now we create our Trajectory object and alter some method parameters via the keyword arguments
                ...
                S = ControlSystem(f, a, b, xa, xb, ua, ub,
                      su=2, sx=2, kx=2, use_chains=False, k=par, sol_steps=100, maxIt=5, dt_sim=0.01)  # k must be a list, dt=0.01
                T_start = time.time()
...
if save_u:
    ...
    U1_data = U1_Umgebung
    u1_place = open(path + '\\U1_data_dt_0.01.plk', 'wb')
    pickle.dump(U1_data, u1_place)
    u1_place.close()

    U2_data = U2_Umgebung
    u2_place = open(path + '\\U2_data_dt_0.01.plk', 'wb')
    pickle.dump(U2_data, u2_place)
    u2_place.close()


    X1_data = X1_Umgebung
    x1_place = open(path + '\\X1_data_dt_0.01.plk', 'wb')
    pickle.dump(X1_data, x1_place)
    x1_place.close()

    X2_data = X2_Umgebung
    x2_place = open(path + '\\X2_data_dt_0.01.plk', 'wb')
    pickle.dump(X2_data, x2_place)
    x2_place.close()

    X3_data = X3_Umgebung
    x3_place = open(path + '\\X3_data_dt_0.01.plk', 'wb')
    pickle.dump(X3_data, x3_place)
    x3_place.close()


    t_data = t
    t_place = open(path + '\\t_data_dt_0.01.plk', 'wb')
    pickle.dump(t_data,t_place)
    t_place.close()

    k_data = K_List_for_Asy
    k_place = open(path + '\\k_data_dt_0.01.plk', 'wb')
    pickle.dump(k_data, k_place)
    k_place.close()