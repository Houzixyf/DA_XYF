...
def f(x, u, par, evalconstr=True):
    ...
    ff = [k*x2,
          k*u1]
    if evalconstr:
        res = k*pe(k, 0, 2) #  pe(k, 0, 10)实际上只要保证后面的Pe(k,x,x)就行，前面乘不乘k没关系。因为需要的是使k在等于kmid时，利用LM算法得到的res为0，而这点只要有pe，就都能实现，所以前面是什么并不会十分影响最终k=kmid。
        ff.append(res)
    return ff
...
par = [1.23]
S = ControlSystem(f, a, b, xa, xb, ua, ub, su=2, sx=2, kx=2, use_chains=False, first_guess=None, maxIt=10, k=par)  # k must be a list
