    ...
    def get_guess(self):
        ...
        if not self.trajectories._old_splines:
            ...
            if self._first_guess is None:
                ...
                cx_cu = False
                if cx_cu:
                    load_sol_for_k = open('d:\\sol_test0.plk', 'rb')
            ...
            # user defines initial value of free coefficients
                ...
                for k, v in sorted(self.trajectories.indep_vars.items(), key = lambda (k, v): k):
                    ...
                    if not (self.sys.states.__contains__(k) or self.sys.inputs.__contains__(k)):# len(k)==7 or len(k)==8: # Z_par_1 or more
                        guess[-self.sys.n_par:] = self._parameters['z_par']
                self.FS.append(guess.tolist())
        else:
            guess = np.empty(0)
