...
class ControlSystem(object):
    ...
    def __init__(self, ff, a=0., b=1., xa=[], xb=[], ua=[], ub=[], constraints=None, **kwargs):
        ...
        self.eqs = CollocationSystem(sys=self.dyn_sys, constraints=constraints, **kwargs)
    ...
    def unconstrain(self, constraints):
        ...
        self._dyn_sys_orig = copy.deepcopy(self.dyn_sys) ##:. contains f_sym, a,b,xa,xb,ua,ub
        ...
        ff_mat = sp.Matrix(self.dyn_sys.f_sym(x, u, par)) ##:: ori, but will be changed at the last of the function
        ...
        for k, v in self.constraints.items():
            ...
            if type(k) == str:
                xk = self.dyn_sys.par[int(k[4])]
                yk = sp.Symbol(xk)
                mk = 4.0 / (v[1] - v[0])
                psik = v[1] - (v[1] - v[0]) / (1. + sp.exp(mk * yk))

                # dpsi = ((v[1]-v[0])*m*sp.exp(m*yk))/(1.0+sp.exp(m*yk))**2
                dpsik = (4. * sp.exp(mk * yk)) / (1. + sp.exp(mk * yk)) ** 2  # dpsi/dyk
                self.dpsi_k = dpsik
                self.mk = mk
                self.v = v
                # replace constrained variables in vectorfield with saturation expression
                # x(t) = psi(y(t))
                ff_mat = ff_mat.replace(sp.Symbol(xk), psik) ##:: yk
            else:
                ...
                # dpsi = ((v[1]-v[0])*m*sp.exp(m*yk))/(1.0+sp.exp(m*yk))**2
                dpsi = (4. * sp.exp(m * yk)) / (1. + sp.exp(m * yk)) ** 2  # dpsi/dyk
                # replace constrained variables in vectorfield with saturation expression
                # x(t) = psi(y(t))
                ff_mat = ff_mat.replace(sp.Symbol(xk), psi) ##:: yk
                ...
                boundary_values[xk] = ((1. / m) * np.log((xa - v[0]) / (v[1] - xa)),  # y(a)
                                       (1. / m) * np.log((xb - v[0]) / (v[1] - xb)))  # y(b)
            # update boundary values for new unconstrained variable

        
        # create a callable function for the new symbolic vectorfield
        ff = np.asarray(ff_mat).flatten().tolist() ##:: yk
        xup = self.dyn_sys.states + self.dyn_sys.inputs + self.dyn_sys.par
        _f_sym = sp.lambdify(xup, ff, modules='sympy') ##:: yk _f_sym(1,1,1,1,1,1) = [5.98687660112452, 5.98687660112452*(-0.05 + 0.981*cos(1))*sin(1)/(0.1*sin(1)**2 + 1.0) + 5.98687660112452/(0.1*sin(1)**2 + 1.0), 5.98687660112452, 5.98687660112452*cos(1)/(0.05*sin(1)**2 + 0.5) + 5.98687660112452*(-0.05*cos(1) + 10.791)*sin(1)/(0.05*sin(1)**2 + 0.5)] is new yk, cause kx2 in old function should be 1.
        def f_sym(x, u, p):
            xup = np.hstack((x,u,p))
        ...
        self.dyn_sys = DynamicalSystem(f_sym, a, b, xa, xb, ua, ub, par=self.dyn_sys.par)

    def f_dpsi(self, yk):
        return ((4. * math.e**(self.mk * yk)) / (1 + math.e**(self.mk * yk)) ** 2) / yk

# Todo: debug!!!!!!!!!!!!
    def f_inv_psi(self, k): # k -> yk
        if (k - self.v[0]) / (self.v[1] - k) <= 0:
            raise FooError('invalid value: x in ln(x) must greater than 0!')
        yk = math.log((k - self.v[0]) / (self.v[1] - k), math.e) / self.mk
        return yk

    def f_psi(self, yk):
        if type(yk) == list:# yk -> k
            k = []
            for i in range(len(yk)):
                k.append(self.v[1] - (self.v[1] - self.v[0]) / (1.0 + np.exp(self.mk * yk[i])))
        else:
            k = self.v[1] - (self.v[1] - self.v[0]) / (1.0 + np.exp(self.mk * yk))
        return k
...
#            if type(k) == str:
#                park = self.dyn_sys.par[int(k[4])]
#                yk_pos = self.dyn_sys.par.index(park)
#                yk_val = self.par[yk_pos]
#                k_val = self.f_psi(yk_val)
#                self.sim_data[-1][yk_pos] = k_val # rechange yk to k
#                self.par[yk_pos] = k_val
            #else:
# iterate over all constraints
        for k, v in self.constraints.items():
            # get symbols of original constrained variable x_k, the introduced unconstrained variable y_k
    ...
    def solve(self, symbeq = False):
        ...
        # self._iterate()
        # self.nIt += 1
        # logging.info("spline parts: {}".format(self.eqs.trajectories.n_parts_x))


        while not self.reached_accuracy and self.nIt < self._parameters['maxIt']:
            self.eqs.trajectories._raise_spline_parts() # raise the number of spline parts
    ...
    def _iterate(self, symbeq = False):
        '''
        ...
        self.eqs.get_guess(f_inv_psi = self.f_inv_psi)
...
#        IPS()
        ...
        sol, par, sk_list = self.eqs.solve(G, DG) ##:: yk len(sol)=free-parameter, type(sol)=<type 'numpy.ndarray'>
        if self.constraints is not None:
            k_plot = self.f_psi(sk_list)
            self.k_list.extend(k_plot)
        self.sk_list.extend(sk_list)
        self.par_yk = par[0]
        logging.debug("yk: %s" % str(par[0]))
        self.par = par ##:: yk
    ...
    def check_accuracy(self):
        ...
        par_yk = np.array([self.par_yk]) ##:: yk
        if self.dyn_sys.n_par > 1:
            for i in range(self.dyn_sys.n_par - 1):
                par_yk = np.hstack((par_yk, self.par[i+1]))
        #par_yk[0] = self.f_inv_psi(par_yk[0]) ##:: yk
        if ierr:
            # calculate maximum consistency error on the whole interval
            maxH = auxiliary.consistency_error((a,b), self.eqs.trajectories.x, self.eqs.trajectories.u, self.eqs.trajectories.dx, self.dyn_sys.f_num, par_yk) ##::  yk
        ...
        # self.par[0] = self.f_psi(par_yk[0]) # because self.par and par_yk share memory
...
class DynamicalSystem(object):
    ...
    def __init__(self, f_sym, a=0., b=1., xa=[], xb=[], ua=[], ub=[], **kwargs):
        self.f_sym = f_sym ##:: ori
        self.a = a
        self.b = b

        self.z_par = kwargs.get('par', [1.0])
        ...
        self.f_num = auxiliary.sym2num_vectorfield(f_sym=self.f_sym, x_sym=self.states, u_sym=self.inputs, p_sym=self.par,
                                                   vectorized=False, cse=False) ##::f_ori
